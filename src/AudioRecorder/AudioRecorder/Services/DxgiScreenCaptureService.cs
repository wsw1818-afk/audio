...
/// <summary>
/// DXGI Desktop Duplication 기반 화면 캡처 서비스
/// GDI+ CopyFromScreen 대신 GPU 메모리에서 직접 캡처하여 WPF 렌더링과 충돌하지 않음
/// </summary>
public class DxgiScreenCaptureService : IDisposable
{
    ...
    private bool InitializeDxgi()
    {
        try
        {
            ...
            // CustomRegion일 때도 전체 모니터 크기로 생성 (부분 복사를 위해)
            ...
            Debug.WriteLine($"DXGI 초기화 완료: {_frameWidth}x{_frameHeight}");
            return true;
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"DXGI 초기화 실패: {ex.Message}");
        ...
        while (_isCapturing)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        Debug.WriteLine("너무 많은 연속 캡처 실패, 재초기화 시도...");
                        CleanupDxgi();
                        Thread.Sleep(100);
                        if (!InitializeDxgi())
                        {
    ...
    private bool CaptureFrame()
    {
            ...
            try
            {
                ...
                lock (_frameLock)
                {
                    ...
                    if (_region.Type == CaptureRegionType.CustomRegion)
                    {
                        ...
                        stride = regionStride; // 출력 stride를 영역 크기로 변경
                    }
                    else
                    {
                        // 전체 화면 캡처
                        ...
                        Marshal.Copy(srcPtr, _currentFrame, 0, size);
                    }
                }
        ...
        {
            // 디바이스 손실 - 재초기화 필요
            Debug.WriteLine($"DXGI 디바이스 손실: {ex.Message}");
            return false;
        }
        finally
        {
    ...
    /// ⚠️ 핫 패스(매 프레임 호출)에서 사용 금지 - 매번 new byte[] 할당으로 GC 압력 증가.
    /// 핫 패스에서는 CopyCurrentFrameTo(byte[] buffer)를 사용하여 사전 할당된 버퍼에 복사할 것.
    /// </summary>
